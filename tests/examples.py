...
class Example:
    ...
    def run_example(self,
                    ...
                    gradient=None,
                    test_criterion=sc.Combine(stop1=sc.Convergence(), stop2=sc.MaxIterations(1000)),
...
class Generate_test:
    ...
    def generate(self):
        for num in range(self.count):
            ...
            print(f"alpha, bravo, charlie: {alpha}, {bravo}, {charlie}")
            fun_gen = (lambda point: alpha * (point[0] ** 2) +
                                     bravo * (point[1] ** 2) +
                                     charlie * point[1] * point[0])
            print("anwser: ", end="")
            print(self.ex.run_example(dimension=2,
                                function=fun_gen,
                                file_name=f'{YOUR_DIR_NAME}/{num}.png',
                                beginning_point=[0.5, 0.5],
                                learning_rate_scheduling=lrs.ArmijoRule(2,
                                                                        function=fun_gen)
                                    )
                  )
